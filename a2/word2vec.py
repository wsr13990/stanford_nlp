...
def naiveSoftmaxLossAndGradient(
	...
	#outsideVectors = 5x3
	#uv = 1x5
	uv = np.dot(outsideVectors,np.transpose(centerWordVec))
	...
	for w in range(np.shape(outsideVectors)[0]):
		if (w == outsideWordIdx):
			gradOutsideVecs[w,:] = np.dot(p[w],centerWordVec) - centerWordVec
		else:
			gradOutsideVecs[w,:] = np.dot(p[w],centerWordVec)
	### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
	...
	for o in outsideWords:
		...
		if word2vecLossAndGradient == naiveSoftmaxLossAndGradient:
			curr_loss, curr_gradCenterVecs,curr_gradOutsideVectors = naiveSoftmaxLossAndGradient(centerWordVectors[c,:],i,outsideVectors,None)
			loss+=curr_loss
			gradCenterVecs[c,:] += curr_gradCenterVecs
			gradOutsideVectors += curr_gradOutsideVectors
		elif word2vecLossAndGradient == negSamplingLossAndGradient:
			curr_loss, curr_gradCenterVecs,curr_gradOutsideVectors = negSamplingLossAndGradient(
				centerWordVectors[c,:],
				i,
				outsideVectors,
				dataset
